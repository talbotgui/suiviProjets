...
- Le collage d'une chaîne JSON de credentials (F02) est validé par schéma avant tout usage ; un contenu malformé ou hors schéma est rejeté avec un message explicite, sans tentative d'interprétation partielle.
...
- `cargo audit` (Rust) et `npm audit` (Angular/npm) sont exécutés systématiquement en intégration continue (pipeline détaillé à l'étape 12) et recommandés en local avant tout commit introduisant une nouvelle dépendance.
...
- Le [journal des modifications](./12_modeleDonnees.md#entités-attributs-et-relations) (`EntréeJournal`, étape 7) journalise la modification d'un seuil ou d'un référentiel ([RG-023](./05_reglesGestion.md#seuils-référentiels-et-historisation)) ainsi que la qualification d'un membre depuis une alerte (déduit de la séquence décrite à [l'étape 8](./13_conceptionDetaillee.md#détail-des-modulescomposants-et-de-leurs-interfaces)). Le changement de politique IA d'un projet vers autorisé suit un mécanisme distinct : il crée une annotation système horodatée et non supprimable ([RG-015](./05_reglesGestion.md#constat-jugement-et-politique-ia)), et non une entrée de ce journal.
...
- Les journaux techniques restent strictement locaux, sans télémétrie ni envoi distant, cohérent avec le principe d'une application entièrement locale ; leur emplacement précis et leur politique de rétention seront fixés à l'étape 12 (poste développeur et supervision).
