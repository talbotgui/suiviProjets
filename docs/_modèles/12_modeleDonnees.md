# Modèle de données

## Sommaire

1. [Entités, attributs et relations](#entités-attributs-et-relations)
2. [Invariants et règles de cohérence](#invariants-et-règles-de-cohérence)
3. [Stratégie de persistance](#stratégie-de-persistance)
4. [Stratégie de migration des données](#stratégie-de-migration-des-données)
5. [Règles de validation des données](#règles-de-validation-des-données)
6. [Matrice de traçabilité](#matrice-de-traçabilité)

## Entités, attributs et relations

*Ce chapitre décrit les entités du modèle de données, leurs attributs et les relations entre elles.*

| entité | attributs | relations |
|---|---|---|
| | | |

## Invariants et règles de cohérence

*Ce chapitre décrit les invariants et règles de cohérence applicables au modèle de données.*

## Stratégie de persistance

*Ce chapitre décrit la stratégie de persistance retenue pour les données de l'application.*

## Stratégie de migration des données

*Ce chapitre décrit la stratégie de migration des données, notamment en cas d'évolution du modèle ou de reprise de données existantes.*

## Règles de validation des données

*Ce chapitre décrit les règles de validation applicables aux données à la saisie ou à l'enregistrement.*

| règle de validation | donnée concernée |
|---|---|
| | |

## Matrice de traçabilité

*Ce chapitre relie chaque entité du modèle de données aux règles de gestion de l'étape 3 dont elle dépend.*

| entité | règle de gestion |
|---|---|
| | |
