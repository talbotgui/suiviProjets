...
# Chapitre 4 — Écran « Commits des membres » : régularité des poussées de code d'un groupe (US-060 / RG-060)
...
Décisions fonctionnelles, issues des questions posées à l'utilisateur les 2026-09-01 et 2026-09-02 :

1. **Source et mode d'appel** : événements de poussée récupérés par membre (`GET /users/:id/events?action=pushed&after=…&sort=desc`), un appel paginé par développeur ; couvre toutes les branches, identité GitLab fiable, horodatage = heure de poussée.
2. **Périmètre** : un groupe applicatif à la fois (sélecteur en tête d'écran). Le roster est celui d'un **groupe GitLab**, dont la référence (chemin ou identifiant) est **saisie sur l'écran, en mémoire de session uniquement** — aucun champ ajouté au modèle. Cette référence est **obligatoire** pour lancer une analyse (il n'existe pas de mode de repli par les membres connus).
3. **Instance GitLab** : l'analyse porte sur la **première instance GitLab déclarée** pour le groupe applicatif. Si le groupe en déclare plusieurs, l'écran affiche un message explicite indiquant que seule la première est interrogée.
4. **Persistance** : résultat en mémoire de session seulement, recalculé à la demande par un bouton « Analyser » ; le tri, le filtrage et toute modification des seuils ne relancent aucun appel réseau.
5. **Volet « commits du soir »** : traité par le seul indicateur de créneau horaire (part des poussées faites dans une plage de soirée paramétrable), sans appel supplémentaire ; le taux d'échec de pipeline est explicitement hors périmètre de ce chapitre.
6. **Visualisation** : tableau de tri (visualisation 1 de l'étude de besoin), à l'exclusion des autres formes proposées (nuage de points, repli par équipe) et **sans frise ni sparkline** par ligne.
7. **Score de risque composite** : somme pondérée normalisée de **trois signaux** — inactivité (jours ouvrés depuis la dernière poussée, rapportés au seuil absolu), **écart à la cadence** (heures depuis la dernière poussée rapportées à la cadence médiane personnelle, elles-mêmes rapportées au multiplicateur), part en soirée. Le second signal est le **ratio silence courant / cadence médiane**, pas un coefficient de variation (abandonné). Les trois pondérations sont paramétrables.
8. **Seuils modifiables par l'utilisateur** : fenêtre d'analyse, seuil absolu de jours ouvrés sans poussée, multiplicateur d'écart à la cadence, **trois pondérations du score de risque**, bornes de la plage de soirée, **fuseau horaire de référence unique**, liste de comptes exclus — tous portés par `parametres.cadenceCommits` et édités depuis l'onglet « Réglages applicatifs » du Paramétrage.
9. **Écran** nommé **« Commits des membres »**, placé dans la sidebar **juste après Obsolescence**.
10. **Jours fériés ignorés** dans le calcul des jours ouvrés (aucun calendrier national codé) : l'utilisateur en tient compte à la lecture.
11. **Aucun avertissement de couverture partielle** : un membre dont aucun événement n'est visible par le jeton apparaît simplement sans activité, à charge pour l'utilisateur d'interpréter cette absence d'information (une portée de jeton insuffisante — voir décision d'architecture n° 4 — se lit alors comme une inactivité).
12. **Attribution au pousseur** : une poussée est comptée à son auteur GitLab ; les commits d'autres auteurs qu'elle transporte ne sont pas réattribués — acceptable pour l'objectif de détection de perte de travail.
13. **Dimension RH assumée** : l'écran restitue des indicateurs nominatifs de rythme de travail. Un **bandeau permanent** sur l'écran, ainsi que la description de la fonctionnalité dans le guide utilisateur et les cas d'usage, rappellent que cette exploitation relève de la responsabilité RH / d'information du personnel de l'organisation utilisatrice (arbitrage humain assumé du 2026-09-02).

Décisions d'architecture :

1. Le **calcul des indicateurs** (dernière poussée, jours ouvrés depuis, cadence médiane, écart à la cadence, part en soirée, score de risque, statut du membre) est une **fonction pure du Moteur de jugement** (`src/app/services/sansetat/jugement/cadence-poussees.utils.ts`), sans effet de bord, testée au seuil 90 %. Elle prend en entrée l'activité brute déjà récupérée, les seuils courants et l'instant de référence ; elle est réévaluée à chaque changement de seuil sans nouvel appel réseau.
...
3. L'**orchestration multi-appels** (une passe de préparation, puis une boucle sur les membres à concurrence limitée, avec progression réactive) est portée par un **Store d'état applicatif** dédié (`services/avecetat/commits-membres/`), sur le modèle explicite de `OrchestrateurCampagneService` : progression tenue dans le Store, boucle côté interface via le **même patron `mergeMap(fn, concurrence)` RxJS** que `OrchestrateurCampagneService.lancerCampagne`, à concurrence `parametres.audit.concurrence`.
4. Le **roster** est obtenu par `GET /groups/:ref/members/all` (paginé, deux à quatre appels pour une centaine de personnes), `:ref` = référence de groupe GitLab saisie sur l'écran. La complétude et la justesse du roster comme des événements dépendent de la **portée du jeton** de l'instance : sur une instance Community Edition avec des dépôts privés, un jeton d'**administration** (ou membre de tous les projets concernés) est nécessaire pour une vue complète ; ce prérequis est rappelé dans le guide utilisateur et `15_normesSecurite.md`. Aucune détection ni aucun avertissement automatique (décision fonctionnelle n° 11).
5. Chaque membre du roster est **classé** (interne, client, partenaire, inconnu) par `StatutMembreUtils` contre les membres connus du groupe, exactement comme la ventilation des membres du dépôt (US-017), via son paramètre de type générique `RegleMembreConnu<TStatut>` (aucun import de `services/avecetat/` depuis le Moteur de jugement). `GET /groups/:ref/members/all` renvoie l'adresse électronique de chaque membre uniquement pour un jeton d'administration : `MembreGroupeGitlab` porte donc un `courriel: Option<String>`, transmis à la classification quand il est présent, ce qui permet aux règles de membre connu de type `email` et `domaineEmail` de s'appliquer ; à défaut, seules les règles `username` sont résolues et les autres membres restent `inconnu`. Une issue `conflit` de `StatutMembreUtils` est traitée comme `inconnu`. **Le filtre de statut du tableau est par défaut sur « tous »** (le roster est déjà celui de l'équipe) ; « internes uniquement » reste un filtre disponible.
...
Chaîne des identifiants à la date de rédaction : documents normatifs à `US-056` / `RG-056` ; `plan_17` chapitre 3 propose `US-057` / `RG-057` (non intégrés) ; `plan_18` propose `US-058` / `US-059` / `US-061` et `RG-058` / `RG-059` / `RG-060` (non intégrés — sa relecture isolée du 2026-09-02 a aligné son `RG` sur son `US-061`) ; `plan_17` chapitre 5 propose `US-062` / `RG-062` (non intégrés). Ce chapitre reprend donc `US-060` / `RG-060`, désormais le premier couple libre (l'asymétrie éventuelle US/RG est explicitement acceptée). Allocation proposée ci-dessous, **à reconfirmer au moment de la qualification effective** ; en cas de consommation concurrente, décaler l'ensemble de ce chapitre en bloc sans réintroduire de trou (principe déjà appliqué en Étape 25 de `plan_16`).

| identifiant | intitulé | type |
|---|---|---|
| US-060 | Consulter, sur l'écran « Commits des membres », la régularité des poussées de code des développeurs d'un groupe sur une fenêtre glissante (tableau de tri, seuils paramétrables) | Consultation — Should have |
| RG-060 | Calcul des indicateurs de régularité de poussée : source (événements de poussée par membre du roster), fenêtre glissante, définition de chaque indicateur (dernière poussée, jours ouvrés depuis, nombre de poussées et de commits, cadence médiane, écart à la cadence, part en soirée), seuils paramétrables, score de risque composite à trois signaux, classement par ce score, périmètre des membres retenus et classification par les membres connus | règle de gestion |
...
- `lister_membres_groupe(url_base, credential, groupe_ref, client) -> Result<Vec<MembreGroupeGitlab>, ErreurConnecteur>` : `GET /groups/{ref}/members/all`, paginé jusqu'à épuisement ou `MAX_PAGES_CONTRIBUTEURS`, sur le modèle de `recuperer_usernames_membres_groupe` (dont la structure de réponse `ReponseMembre` est étendue pour capter `id`, `name`, `state` et `email`) ; `groupe_ref` encodé via l'utilitaire d'encodage de segment déjà employé pour les refs. Ne retient que les membres `state == "active"`. `MembreGroupeGitlab { id: u64, username: String, nom: String, courriel: Option<String> }` — `courriel` renseigné uniquement quand l'API le retourne (jeton d'administration).
- `lister_projets_groupe(url_base, credential, groupe_ref, client) -> Result<Vec<ProjetGroupeGitlab>, ErreurConnecteur>` : `GET /groups/{ref}/projects?simple=true&include_subgroups=true&per_page=100`, paginé jusqu'à épuisement ou `MAX_PAGES_PROJETS` (constante existante, `gitlab.rs`) ; `ProjetGroupeGitlab { id: u64, chemin: String }` (`path_with_namespace`). Sert à afficher un nom de dépôt lisible dans le tableau ; un identifiant de dépôt hors périmètre du groupe reste affiché tel quel.
- `lister_evenements_poussees(url_base, credential, utilisateur_id, apres_date, client) -> Result<Vec<EvenementPoussee>, ErreurConnecteur>` : `GET /users/{id}/events?action=pushed&after={apres_date}&per_page=100&sort=desc`, paginé du plus récent au plus ancien jusqu'à épuisement ou `MAX_PAGES_EVENEMENTS_POUSSEE`. `sort=desc` (défaut de l'API) garantit que, pour un membre très actif, ce sont les poussées **les plus anciennes** qui sont tronquées, jamais les plus récentes — l'indicateur « dernière poussée » et la cadence récente restent exacts. `apres_date` au format `YYYY-MM-DD` fixé à la veille du début de fenêtre (le paramètre `after` de GitLab est exclusif et à granularité de jour) ; le filtrage fin sur l'instant exact de début de fenêtre est fait côté interface. Ignore les événements dont `push_data.action == "removed"` ou `push_data.commit_count == 0` (suppression de branche). `EvenementPoussee { horodatage: String, projet_id: u64, ref_poussee: String, nombre_commits: u32 }` (depuis `created_at` et `push_data.{ref, commit_count}`).
...
### Commandes de la Façade — nouveau module `src-tauri/src/commandes/commits_membres.rs`

Déclaration `pub(crate) mod commits_membres;` dans `src-tauri/src/commandes.rs` (ordre alphabétique). En-tête de mention IA, documentation de module `//!` référençant US-060 et RG-060.
...
- `preparer_analyse_commits_membres(instance: Instance, groupe_gitlab: String, etat: State<'_, EtatSession>) -> Result<PreparationAnalyseCommitsMembres, ErreurConnecteur>` : résout le credential mémorisé de l'instance (helper `credential_instance`, extrait de `commandes/audit.rs` vers un module partagé `commandes/commun.rs`) ; appelle `lister_membres_groupe` puis `lister_projets_groupe`. Retourne `PreparationAnalyseCommitsMembres { membres: Vec<MembreGroupeGitlab>, projets: Vec<ProjetGroupeGitlab> }`. `groupe_gitlab` est obligatoire (aucun mode de repli par les membres connus).
...
Les deux commandes rejettent une instance de type Sonar (`ErreurConnecteur::ReponseInattendue`, défense en profondeur, comme les commandes d'audit GitLab existantes). Le paramètre `cible` passé à `consigner_appel_connecteur` ne porte que des données non nominatives (identifiant numérique d'utilisateur GitLab, référence de groupe), jamais une adresse électronique.

### Enregistrement

`src-tauri/src/lib.rs` : ajouter `commandes::commits_membres::preparer_analyse_commits_membres` et `commandes::commits_membres::lister_evenements_poussees_membre` au `tauri::generate_handler!`.

Note : ces deux commandes n'écrivent rien sur disque et ne demandent pas le mot de passe du fichier (consultation pure) ; la commande `definir_parametres_cadence_commits` de la Partie D, elle, écrit `parametres` et suit à ce titre le patron complet de `definir_concurrence_audit` (ressaisie du mot de passe, RG-002).
...
`src/app/services/sansetat/commandes/facade-commandes.service.ts` (ou une façade dédiée `facade-commits-membres.service.ts` si la taille le justifie) : deux méthodes `public async`, type de retour explicite, JSDoc, sur le modèle de `interrogerContributeurs` :

- `preparerAnalyseCommitsMembres(parametres): Promise<PreparationAnalyseCommitsMembres>` déléguant à `invoke('preparer_analyse_commits_membres', …)` ;
...
Le paramètre d'entrée `Instance` réutilise le miroir de `Instance` déjà présent dans `types-facade.ts` (comme `interrogerContributeurs`), sans généricité : la règle de généricité de la Façade ne vise que les types possédés exclusivement par `avecetat/etat/`, ce qui n'est pas le cas d'`Instance`. Aucun type de `MembreConnu` ne transite plus par ces commandes (la classification est faite côté interface — décision d'architecture n° 5).

`src/app/services/sansetat/commandes/bouchon/` : nouveau `bouchon-commits-membres.utils.ts` et son spec ; enregistrement des deux commandes dans le `Set` de `bouchon-commandes.utils.ts`. Jeu de démonstration **déterministe et rejouable** : quatre développeurs synthétiques — un régulier (poussées quotidiennes), un silencieux depuis une dizaine de jours ouvrés, un « du soir » (poussées concentrées après 19 h), un irrégulier (rafales puis silence) — et deux à trois dépôts. Les horodatages de poussée sont **calculés par décalage fixe depuis `Date.now()`** au moment de l'analyse (ex. « dernière poussée du régulier = maintenant − 2 h », « du silencieux = maintenant − 14 jours ») : l'ordre des lignes et l'état des alertes sont donc stables d'une exécution à l'autre, sans valeur aléatoire (même principe que l'aléa borné du bouchon Sonar, couvert par des assertions à bornes tolérantes). Un délai artificiel fixe sur `lister_evenements_poussees_membre` permet d'exercer réellement l'indicateur de progression.

### Types et Store d'orchestration — `src/app/services/avecetat/commits-membres/`

`commits-membres.types.ts` : `EvenementPoussee`, `PreparationAnalyseCommitsMembres`, `ActivitePousseesDeveloppeur { membre: { id: number; username: string; nom: string; courriel: string | null }; evenements: readonly EvenementPoussee[] }`, `LigneCadencePoussees` (cf. util ci-dessous), toutes en `readonly`, miroir strict des structures Rust en `camelCase`.

`commits-membres.service.ts` : Store stateful, `@Injectable({ providedIn: 'root' })`, JSDoc.
...
- `public async analyser(groupeApplicatifId: string, groupeGitlab: string): Promise<void>` : retient la **première instance de type GitLab** déclarée pour le groupe applicatif (via `DonneesApplicationService`) ; si le groupe en déclare plusieurs, expose un indicateur (`plusieursInstancesGitlab: boolean`) que l'écran restitue en message. Appelle `preparerAnalyseCommitsMembres` ; puis boucle sur les membres via le **même patron `mergeMap(fn, concurrence)` RxJS** que `OrchestrateurCampagneService.lancerCampagne` (`orchestrateur-campagne.service.ts`), à concurrence `parametres.audit.concurrence` (repli identique à `extraireConcurrence`), en appelant `listerEvenementsPousseesMembre` et en mettant à jour `progression` après chaque membre ; agrège dans `activiteBrute` et fixe `instantAnalyse`. Toute erreur d'un membre est consignée via `NotificationService` et n'interrompt pas la boucle ; une erreur de préparation interrompt l'analyse. `enCours` encadre l'ensemble.
- `public readonly lignes: Signal<readonly LigneCadencePoussees[]>` : `computed` appelant `CadencePousseesUtils.analyser(this.activiteBrute() ?? [], seuils, this.cheminsDepotsParId(), reglesMembresConnus, maintenant)` où `seuils` provient de `donneesApplication.racine()?.parametres.cadenceCommits` (repli sur les valeurs par défaut) et `reglesMembresConnus` est la projection des membres connus du groupe vers la forme générique `RegleMembreConnu<StatutDeveloppeur>`. Recalcul automatique à tout changement de seuil, **sans** relancer `analyser`.
...
- `public static analyser(activite, seuils, cheminsDepotsParId, reglesMembresConnus, maintenantIso): readonly LigneCadencePoussees[]` : une ligne par développeur non exclu (`seuils.comptesExclus`), filtrée à la fenêtre `[maintenant − fenetreJours ; maintenant]`, triée par `scoreRisque` décroissant (ordre par défaut ; le composant applique ensuite le tri de colonne choisi par l'utilisateur).
- Méthodes privées pures :
  - `joursOuvresEntre(depuisIso, jusquaIso)` : lundi-vendredi, jours fériés ignorés (décision fonctionnelle n° 10), même jour → 0, vendredi → lundi → 1.
  - `medianeIntervallesHeures(horodatagesTries)` : médiane des écarts entre poussées consécutives, sur les horodatages **triés en ordre croissant** par la méthode (les événements arrivent du connecteur en ordre décroissant).
  - `partCreneauSoiree(evenements, seuils)` : heure locale de chaque `horodatage` via `Intl.DateTimeFormat(seuils.fuseauHoraire, { hour: 'numeric', hour12: false })`, déterministe, fuseau explicite, heure d'été gérée ; **`try/catch` autour de la construction du formateur** : un `RangeError` (fuseau invalide malgré la validation amont) fait basculer sur `'UTC'` et n'interrompt pas le calcul. Plage à repli circulaire quand `heureDebutSoiree > heureFinSoiree`.
  - `scoreComposite(...)` : somme pondérée puis normalisée par la somme des poids, de **trois signaux bornés dans `[0 ; 1]`** — (1) inactivité = `min(1, joursOuvresDepuis / seuilJoursOuvresSansPoussee)` ; (2) écart à la cadence = `min(1, (heuresDepuisDernierePoussee / cadenceMedianeHeures) / multiplicateurEcartCadence)` — **ratio silence courant / cadence médiane** (décision fonctionnelle n° 7), pas un coefficient de variation ; (3) part en soirée = `partSoiree`. Un signal non calculable (`donneesInsuffisantes` pour le n° 2) vaut `0` et son poids est retiré de la normalisation. Pondérations lues depuis `seuils` (`ponderationInactivite` / `ponderationEcartCadence` / `ponderationSoiree`).
  - `alertes` : `'inactivite'` si signal 1 = 1, `'ecartCadence'` si le ratio du signal 2 dépasse `multiplicateurEcartCadence`, `'soiree'` si `partSoiree` dépasse un seuil documenté (constante, décision arbitraire).
- `ecartCadence` = `heuresDepuisDernierePoussee / cadenceMedianeHeures` ; `donneesInsuffisantes` vrai (et `cadenceMedianeHeures` / `ecartCadence` à `null`) quand le développeur a strictement moins de deux poussées sur la fenêtre.
- Le statut vient de `StatutMembreUtils` (import interne au Moteur de jugement, aucun accès `avecetat/`), résolu à partir de `membre.username` et de `membre.courriel` (quand non nul) ; une issue `conflit` est ramenée à `'inconnu'`.

### Spec `cadence-poussees.utils.spec.ts`

Chaque branche exercée (seuil 90 %) : fenêtre vide → aucune ligne ; un seul développeur régulier → cadence et écart cohérents ; développeur à une seule poussée → `donneesInsuffisantes`, signal n° 2 retiré de la normalisation ; jours ouvrés (week-end sauté, même jour → 0, vendredi → lundi → 1) ; médiane sur 0 / 1 / 2 / n intervalles ; ratio silence / cadence médiane borné à 1 ; part en soirée avec plage circulaire (19 h → 7 h) et deux fuseaux distincts, dont un cas d'heure d'été ; **fuseau invalide → bascule sur `'UTC'` sans exception** ; `comptesExclus` retire la ligne ; score composite monotone (silence plus long ⇒ score plus élevé, toutes choses égales) ; une pondération portée à `0` neutralise le signal correspondant ; tri décroissant stable ; classification interne / client / partenaire / inconnu (règles `username` **et** `email` / `domaineEmail` quand `courriel` est fourni) ; issue `conflit` ramenée à `inconnu`.

## 6. Partie C — Écran « Commits des membres »

### Route et branchement dans la coquille

- Nouveau répertoire `src/app/ecrans/commits-membres/`.
- Route `/commits-membres` (avec `withHashLocation()` déjà en place) ajoutée à `app.routes.ts` en `component:` **eager** (`SqmCommitsMembresComponent`), comme tous les écrans existants.
- Nouvelle entrée de la sidebar, **juste après Obsolescence** (avant Liste de travail), libellé **« Commits des membres »** (décision actée n° 9). L'ordre de la sidebar est « unique et fixe » : l'insertion à cette position est actée, sa formalisation dans `08_arborescenceNavigation.md` reste soumise à la validation humaine de l'incrément 1.

### `commits-membres.component.ts`

- Classe `SqmCommitsMembresComponent`, `selector: 'app-commits-membres'`, standalone, `ChangeDetectionStrategy.OnPush`, en-tête de mention IA.
- Dépendances : `inject(CommitsMembresService)`, `inject(DonneesApplicationService)`, `inject(NotificationService)`.
...
- Signaux de tri et de filtrage, tous locaux (aucun appel réseau) : `colonneTri`, `sensTri` (défaut : score de risque décroissant), `filtreStatut` (défaut : **`'tous'`**, décision d'architecture n° 5), `filtreTexte`, `seulementAlertes`.
...
- `computed` `messagePlusieursInstances()` : non vide quand le groupe sélectionné déclare plusieurs instances GitLab (`store.plusieursInstancesGitlab()`).
- Compteurs d'en-tête dérivés (`nombreDeveloppeurs()`, `nombreEnAlerte()`), avec JSDoc et type de retour explicite.
- `public groupesAnalysables()` : groupes du fichier possédant au moins une instance de type GitLab.
...
### `commits-membres.component.html`

- **Bandeau permanent de dimension RH** (décision fonctionnelle n° 13), non masquable : « Cet écran présente des indicateurs nominatifs de rythme de travail. Leur exploitation relève de la responsabilité RH et d'information du personnel de votre organisation. »
- Bandeau de commande : sélecteur de groupe (`groupesAnalysables()`), champ texte « Groupe GitLab (chemin ou identifiant) » (**obligatoire**), bouton « Analyser », horodatage de la dernière analyse (« Analyse du … »), rappel « Les commits non poussés ne sont pas visibles », et `messagePlusieursInstances()` le cas échéant (« Le groupe déclare plusieurs instances GitLab ; seule la première (`<nom>`) est interrogée. »).
...
- `@else if (store.instantAnalyse(); as instant) { <barre de filtres> <table> }` :
  - barre de filtres : statut (défaut **tous**), recherche texte, case « seulement les lignes en alerte », lien « Ajuster les seuils » vers Paramétrage › Réglages applicatifs.
  - tableau trié : colonnes **Développeur**, **Statut**, **Dernière poussée** (date et heure locales), **Dépôt**, **Jours ouvrés depuis**, **Poussées / commits (fenêtre)**, **Cadence médiane**, **Écart à la cadence**, **Part en soirée**, **Score de risque**. En-têtes cliquables (tri). Badge(s) d'alerte par ligne. Ligne « données insuffisantes » quand `donneesInsuffisantes`. **Aucune frise ni sparkline** (décision fonctionnelle n° 6). *Amendé à la relecture (Étape 40 du rapport) : la mention initiale « cellules colorées par rapport aux seuils » est abandonnée — ces indicateurs n'ont pas de grille de seuils de couleur au sens de RG-022, la mise en évidence se limite aux badges d'alerte et au grisé « données insuffisantes ».*
- `@else { <état initial : « Sélectionnez un groupe, saisissez la référence du groupe GitLab, puis lancez une analyse. » > }`.

### `commits-membres.component.scss`

Mise en forme du tableau (en-têtes collants, débordement horizontal `deborde-auto`, indicateur de colonne triée), badges d'alerte, grisé discret de la ligne « données insuffisantes ». Aucune règle de couleur métier codée ici (cf. amendement ci-dessus : pas de coloration conditionnelle des cellules).

### `commits-membres.component.spec.ts` (Jest)

`jest.mock('@tauri-apps/api/core', …)` ; Store alimenté par une `activiteBrute` de test. Assertions : le tri et le filtrage modifient `lignesAffichees()` **sans** appeler `invoke` ; changement d'un seuil (`parametres.cadenceCommits`) recolore / réordonne le tableau sans nouvel `invoke` ; filtre statut par défaut = `tous` ; bandeau RH permanent présent et non masquable ; message « plusieurs instances GitLab » affiché quand le groupe en déclare plus d'une ; état initial et état de progression ; bouton « Analyser » désactivé tant que groupe ou référence GitLab manquent. Suivi du nombre de méthodes jamais appelées (composant de présentation).
...
| `ponderation_inactivite` | `f64` | `0.5` | poids du signal d'inactivité dans le score de risque composite |
| `ponderation_ecart_cadence` | `f64` | `0.3` | poids du signal « ratio silence courant / cadence médiane » |
| `ponderation_soiree` | `f64` | `0.2` | poids du signal de part en soirée |
...
Chaque champ porte un `#[serde(default = "…")]` propre (comme `Referentiels::motif_nommage_branches`, `racine.rs`), pas seulement `#[serde(default)]` sur le champ `cadence_commits` de `Parametres` : un objet partiel dans un fichier édité à la main récupère alors les défauts signifiants plutôt que `0` / `""`. `impl Default` reste fourni pour la cohérence. Toutes ces valeurs par défaut sont des **décisions arbitraires** (aucun texte normatif ni `docs/01_besoin/exemple-donnees.json` ne les fixe) : documentées en commentaire au plus près des fonctions de défaut et signalées comme telles dans le rapport de développement.

`src-tauri/src/commandes/parametrage.rs` : nouvelle commande `definir_parametres_cadence_commits(parametres: CadenceCommits, mot_de_passe: String, …)` sur le **gabarit complet** de `definir_concurrence_audit` — validation de forme (fenêtre 7–90 jours, seuil ≥ 1, multiplicateur ≥ 1, chaque pondération dans `[0 ; 1]`, heures 0–23, **`fuseau_horaire` : identifiant IANA reconnu**, cf. ci-dessous), puis `verifier_avant_ecriture` + écriture dans `parametres.cadence_commits` + `sauvegarder_fichier` (donc **ressaisie du mot de passe du fichier, RG-002**), entrée de journal des modifications, journalisation technique début / fin. Enregistrement dans `lib.rs`.

Validation du fuseau horaire — deux niveaux (norme de sécurité « validé côté UI et revalidé côté cœur natif ») : côté interface, la valeur est choisie dans une liste issue de `Intl.supportedValuesOf('timeZone')` ; côté cœur natif, contrôle qu'elle appartient à une liste embarquée de fuseaux IANA (ou, à défaut de dépendance dédiée, contrôle de forme strict `Region/Ville`). En dernier recours, la fonction pure du Moteur de jugement bascule sur `'UTC'` sans lever (cf. Partie B).

### Interface

`src/app/services/avecetat/etat/types-donnees.ts` : `CadenceCommits` miroir en `camelCase`.

`src/app/services/avecetat/etat/donnees-application.service.ts` : `public async definirParametresCadenceCommits(parametres: CadenceCommits, motDePasse: string): Promise<ResultatMutationAdministration>` avec JSDoc, sur le modèle de `definirConcurrenceAudit` (le type de retour est `ResultatMutationAdministration`, comme les autres mutations de réglages).

`src/app/ecrans/parametrage/reglages-applicatifs/reglages-applicatifs-parametrage.component.ts` et son template : nouvelle section « Commits des membres », formulaire des dix réglages, validation miroir de la validation native, message de portée « s'applique au prochain calcul de l'écran « Commits des membres » » (US-040). `src/app/services/sansetat/commandes/bouchon/bouchon-parametrage.utils.ts` : cas de la nouvelle commande.
...
| document | nature de la mise à jour |
|---|---|
| [`04_casUsage.md`](../02_documentation/04_casUsage.md) | Nouvelle ligne US-060 : persona Camille, type Consultation, priorité Should have ; critères d'acceptation = référence de groupe GitLab obligatoire, tableau par développeur sur fenêtre glissante, indicateurs de dernière poussée / jours ouvrés depuis / cadence médiane / écart à la cadence / part en soirée / score de risque, tri et filtrage sans nouvel appel réseau, seuils modifiables depuis le Paramétrage, résultat non persisté, rappel de la non-visibilité des commits locaux, **bandeau permanent de dimension RH**. RG associée RG-060. Compléter les critères d'acceptation d'US-034 (nouveaux réglages, ressaisie du mot de passe) et d'US-040 (message de portée « au prochain calcul »). Mise à jour de la matrice de couverture. |
| [`05_reglesGestion.md`](../02_documentation/05_reglesGestion.md) | Nouvelle RG-060 : source (événements de poussée par membre du roster ; roster = membres du groupe GitLab désigné par sa référence, sur la première instance GitLab du groupe applicatif), fenêtre glissante paramétrable, définition exacte de chaque indicateur, jours ouvrés (lundi-vendredi, jours fériés ignorés), plage de soirée à repli circulaire dans un fuseau paramétrable, **score de risque composite à trois signaux** (inactivité, ratio silence courant / cadence médiane, part en soirée) et pondérations paramétrables, classement par ce score, **filtre de statut par défaut « tous »**, classification par les membres connus (RG-006 à RG-010, règles `email` / `domaineEmail` résolues quand le jeton renvoie l'adresse), concurrence des appels (RG-017), catégories d'anomalie (RG-021), aucune détection de couverture partielle, résultat jamais persisté. Articulation avec RG-031 (nouveaux réglages applicatifs) et RG-002 (ressaisie du mot de passe pour la modification des seuils). Complément des matrices de traçabilité RG × US. |
| [`08_arborescenceNavigation.md`](../02_documentation/08_arborescenceNavigation.md) | Ajout de l'écran « Commits des membres » dans l'arborescence et dans l'ordre de la sidebar **juste après Obsolescence, avant Liste de travail** (insertion actée, formalisation soumise à validation humaine) ; nouvelle ligne dans la matrice écrans / user stories avec US-060 ; règle de navigation : accès depuis la sidebar uniquement, lien contextuel « Ajuster les seuils » vers Paramétrage. |
| [`09_maquettes.md`](../02_documentation/09_maquettes.md) | Nouvelle section « ### Commits des membres » : **bandeau permanent de dimension RH**, bandeau de commande (sélecteur de groupe, champ Groupe GitLab obligatoire, bouton Analyser, horodatage, message « plusieurs instances »), indicateur de progression, barre de filtres, tableau trié à dix colonnes avec badges d'alerte (sans frise ni sparkline, sans coloration de cellule — cf. amendement §6), états initial et « analyse en cours ». Section « ### Paramétrage » : nouvelle zone « Commits des membres » de l'onglet Réglages applicatifs. |
| [`12_modeleDonnees.md`](../02_documentation/12_modeleDonnees.md) | Description de `parametres.cadenceCommits` (dix champs, défauts par champ, décision arbitraire) ; **`versionSchema` incrémentée** et nouveau palier de migration à transformation nulle (valeur exacte non fixée, coordination `plan_18`) ; note que le résultat d'analyse de l'écran « Commits des membres » est une vue calculée non persistée. |
| [`13_conceptionDetaillee.md`](../02_documentation/13_conceptionDetaillee.md) | Inventaire des commandes de la Façade : `preparerAnalyseCommitsMembres` et `listerEvenementsPousseesMembre` (entrées / sorties, consultation pure, journalisation début / fin, rejet d'une instance Sonar) et `definirParametresCadenceCommits` (écriture disque, ressaisie du mot de passe). Connecteur GitLab : `lister_membres_groupe`, `lister_projets_groupe`, `lister_evenements_poussees`. Ligne « UI — Moteur de jugement » : ajout de `CadencePousseesUtils` (fonction pure). Nouveau Store `CommitsMembresService` (orchestration multi-appels à concurrence limitée, patron RxJS de l'Orchestrateur de campagne). Nouvel écran `SqmCommitsMembresComponent`. |
| [`15_normesSecurite.md`](../02_documentation/15_normesSecurite.md) | Rappel : les indicateurs sont nominatifs et vivent en mémoire de session uniquement ; aucun écrit disque, aucune télémétrie, aucun envoi distant ; credential en en-tête `PRIVATE-TOKEN` ; `cible` de journalisation sans adresse électronique. **Prérequis de portée du jeton** : une vue complète du roster et des événements suppose un jeton d'administration (ou membre de tous les projets), une portée insuffisante se lisant en inactivité. **Note de vigilance RH** : l'écran expose des indicateurs de rythme de travail individuels ; sa mise à disposition et son usage relèvent de la responsabilité RH / d'information du personnel de l'organisation utilisatrice (choix humain assumé), matérialisé par un bandeau permanent sur l'écran et un paragraphe dédié du guide utilisateur. |
| [`16_normesTests.md`](../02_documentation/16_normesTests.md) | `cadence-poussees.utils` relève du Moteur de jugement (fonctions pures, seuil 90 %) ; les trois fonctions de connecteur relèvent des clients d'API (réponses HTTP simulées, chaque catégorie RG-021 testée, jamais d'appel réel en CI) ; `SqmCommitsMembresComponent` relève des composants de présentation (suivi du nombre de méthodes jamais appelées) ; une étape est ajoutée au parcours de bout en bout (assertions sur l'ordre et la présence des lignes, jamais sur des magnitudes) ; un scénario de charge est ajouté (analyse simulée à ~100 membres, concurrence RNF-004, rendu du tableau sous RNF-001). |
...
| [`guide-utilisateur.md`](../guide-utilisateur.md) | Nouveau paragraphe « Commits des membres » : objet de l'écran, **dimension RH et responsabilité de l'organisation utilisatrice**, prérequis de jeton d'administration, lecture du tableau, réglage des seuils, limites (commits non poussés invisibles, portée du jeton se lisant en inactivité). Capture `docs/assets/captures/` à générer hors pipeline — à signaler, non bloquant. |
| [`04_rapports/rapportDeDeveloppement.md`](../04_rapports/rapportDeDeveloppement.md) | Consigner les décisions arbitraires à valider par un humain : valeurs par défaut de `parametres.cadenceCommits` (fenêtre 28 jours, seuil 3 jours ouvrés, multiplicateur 2, pondérations 0,5 / 0,3 / 0,2, plage 19 h – 7 h, fuseau « Europe/Paris »), seuil d'alerte « soirée », borne `MAX_PAGES_EVENEMENTS_POUSSEE`, réutilisation de `MAX_PAGES_PROJETS` / `MAX_PAGES_CONTRIBUTEURS`, jours fériés ignorés, attribution au pousseur, absence d'avertissement de couverture partielle, filtre de statut par défaut « tous », bascule `'UTC'` sur fuseau invalide. |
...
- Tests unitaires Rust : connecteur (`lister_membres_groupe`, `lister_projets_groupe`, `lister_evenements_poussees`) avec client HTTP simulé, chaque catégorie RG-021 et la pagination bornée ; capture de `email` sur `lister_membres_groupe` quand présent ; `sort=desc` respecté ; `CadenceCommits` (défauts par champ) et la validation de `definir_parametres_cadence_commits` (dont le fuseau invalide rejeté) ; non-régression de `cargo test` et du chaînage de migration (nouveau palier de transformation nulle).
- Tests unitaires Jest, nouveaux specs : `cadence-poussees.utils.spec.ts` (seuil 90 %), `commits-membres.service.spec.ts` (orchestration, concurrence, progression, erreur partielle, recalcul sur changement de seuil sans ré-appel), `commits-membres.component.spec.ts` (tri / filtrage sans `invoke`, colonnes, états), `bouchon-commits-membres.utils.spec.ts`.
...
- Test de bout en bout Playwright (`e2e/parcours-complet.spec.ts`) : nouvelle étape après un écran existant — navigation vers « Commits des membres », vérification du bandeau RH, sélection d'un groupe, saisie de la référence de groupe GitLab bouchonnée, clic « Analyser », attente du tableau, clic sur un en-tête de colonne (le tri s'applique, aucun nouvel appel). Assertions **sur l'ordre et la présence** uniquement (le développeur « silencieux » du jeu de démonstration apparaît en alerte et devant le développeur « régulier » au tri par score), jamais sur des magnitudes — les horodatages du bouchon sont relatifs à `Date.now()`. Capture d'écran si le parcours en produit une par écran.
...
1. **Documents normatifs** (section 9) : livrés et validés avant tout code ; confirmation que `US-060` et `RG-060` sont libres au moment de la qualification, et coordination avec `plan_18` de la valeur du palier de migration (les deux plans incrémentent `versionSchema`).
2. **Cœur natif — connecteur et commandes d'analyse** (Partie A) : trois fonctions de connecteur, module `commandes/commits_membres.rs`, extraction de `credential_instance` vers `commandes/commun.rs`, enregistrement, tests Rust. Fige le contrat des structures brutes en `camelCase`.
3. **Paramètre `parametres.cadenceCommits`** (Partie D) : modèle (défauts par champ) et `Default`, incrément de `VERSION_SCHEMA_COURANTE` + palier de migration à transformation nulle, commande `definir_parametres_cadence_commits` (avec ressaisie du mot de passe), méthode du Store d'état, section de l'onglet Réglages applicatifs, bouchon parametrage, tests. Indépendant de l'incrément 2.
4. **Façade TypeScript, bouchon et Store d'orchestration** (Partie B, hors Moteur de jugement) : méthodes de façade, `bouchon-commits-membres.utils.ts`, `CommitsMembresService`, types miroir.
...
1. `cd src-tauri && cargo test` : connecteur, validation de commande, chaînage du nouveau palier de migration.
2. `cd src-tauri && cargo clippy --all-targets -- -D warnings`.
...
6. `npm run build` puis `cd src-tauri && cargo build --locked`.
7. `npm start` (front seul, façade bouchonnée) : ouverture de **Commits des membres**, bandeau RH visible, sélection d'un groupe du jeu de démonstration, saisie de la référence de groupe GitLab bouchonnée, « Analyser » — progression visible, tableau peuplé, développeur « silencieux » en tête après tri par score, tri d'une autre colonne sans rechargement, filtre de statut par défaut « tous », ajustement d'un seuil dans **Paramétrage › Réglages applicatifs** (avec ressaisie du mot de passe du fichier) puis retour : le tableau se recolore et se réordonne sans nouvel appel.
8. `npm run test:e2e` : parcours complet incluant la nouvelle étape.
...
3. La génération est intégrée à la campagne : l'orchestrateur appelle la nouvelle commande une fois par source Sonar, dans la boucle qui appelle déjà `interrogerDerniereAnalyse` ; les montées d'un projet remontent dans la valeur de retour de `auditerProjet` (comme le verdict et le résultat de brouillon), et `lancerCampagne` les agrège pour les passer à `enregistrerBrouillon` — commande qui sauvegarde déjà le fichier —, laquelle upsert les annotations système avant d'écrire. Aucun nouvel orchestrateur, aucun champ ajouté au brouillon persisté.
...
| identifiant | intitulé | type |
|---|---|---|
| US-062 | Visualiser les montées de version du serveur Sonar en repères verticaux sur le graphique d'évolution, et masquer les repères par catégorie | Consultation — Could have |
| RG-062 | Détection des montées de version Sonar (événements `SQ_UPGRADE` de `project_analyses/search`), matérialisation en annotation système de portée projet (catégorie `monteeVersionSonar`), idempotence par identifiant dérivé, dédoublonnage inter-projets à l'affichage, masquage des repères verticaux par catégorie | règle de gestion |

RG-062 s'articule avec US-019 / RG-033 (cycle de vie d'une annotation, dont « une annotation système n'est jamais supprimable », propriété directement mobilisée ici), RG-015 (précédent d'annotation système, politique IA), US-016 (le graphique d'évolution réutilisable, dont le masquage des repères par catégorie modifie le comportement décrit dans les critères d'acceptation), RG-021 (catégories d'anomalie des clients d'API, appliquées au nouvel appel), RG-046 (repère vertical du premier audit régulier, même mécanisme) et RG-017 (concurrence des appels de campagne, réutilisée). Les identifiants intermédiaires US-057 à US-061 et RG-057 à RG-061 sont eux aussi consommés par les chapitres 3 et 4 du présent plan et par `plan_18`, tous non encore intégrés.
...
`pub(crate) async fn interroger_montees_version(url_base, credential, id_externe, client) -> Result<Vec<MonteeVersionSonar>, ErreurConnecteur>` : `GET /api/project_analyses/search` avec `project={id_externe}`, `category=SQ_UPGRADE`, `ps=TAILLE_PAGE_ANALYSES_HISTORIQUE`, paginé (`p`) jusqu'à épuisement ou `MAX_PAGES_ANALYSES_HISTORIQUE` (les deux constantes existantes réutilisées, aucune valeur littérale réintroduite). Gestion des statuts : 401 → `ErreurConnecteur::AuthentificationRefusee`, 403 → `ErreurConnecteur::DroitsInsuffisants`, tout autre non-2xx (dont 404 projet introuvable, volontairement fondu comme pour la fonction sœur, le module Sonar n'ayant pas de variante `RefIntrouvable`) → `ErreurConnecteur::ReponseInattendue`, erreur réseau via `erreur_depuis_reqwest` — strictement le patron de `interroger_derniere_analyse`. Pour chaque analyse, chaque `event` de `category == "SQ_UPGRADE"` produit un `MonteeVersionSonar { version: event.name, date: analyse.date }`. Résultat trié par date croissante et dédoublonné sur `version` (première date conservée). Un projet sans aucun événement de ce type — ou jamais analysé — renvoie `Ok(vec![])`, ce n'est pas une erreur.
...
Tests `#[cfg(test)]` (client HTTP simulé, jamais d'appel réel — [norme de tests](../../.claude/rules/11-normes-tests.md#tests-des-clients-dapi)) : réponse nominale multi-pages à deux montées ; filtrage des événements non `SQ_UPGRADE` (profil qualité, version de projet ignorés) ; dédoublonnage sur `version` ; réponse vide → `Ok(vec![])` ; une réponse par catégorie d'anomalie RG-021 (auth refusée 401, droits insuffisants 403, instance injoignable, délai dépassé, réponse inattendue — cette dernière couvrant aussi le 404 « projet introuvable », comme pour `interroger_derniere_analyse`).

### Commande fine dans `src-tauri/src/commandes/audit.rs`

Nouvelle commande `interroger_montees_version_sonar(instance, id_externe, etat)`, sur le patron de `interroger_derniere_analyse` : elle renvoie `Result<Vec<MonteeVersionSonar>, ErreurConnecteur>` **brut** (le cœur natif ne construit pas de variante succès / échec ; c'est la façade TypeScript qui enveloppe en `{ type: 'succes' | 'echec' }`, comme pour la fonction sœur). Résolution du credential mémorisé de l'instance, journalisation technique début / fin (`consigner_debut_commande` / `consigner_fin_commande`, obligatoire) et journalisation de l'appel connecteur, rejet d'une instance non-Sonar (`ReponseInattendue`, défense en profondeur comme les commandes Sonar existantes). Enregistrement dans `src-tauri/src/lib.rs` (`tauri::generate_handler!`).
...
`pub(crate) fn synchroniser_annotations_montee_version(donnees: &mut DonneesRacine, montees_par_projet: &[MonteeVersionSonarProjet], horodatage: &str) -> Vec<String>` : pour chaque projet et chaque montée, calcule `id = format!("montee-version-sonar-{}", empreinte(&montee.version))` (`empreinte` : minuscules, caractères non alphanumériques remplacés par `-` ; collision théorique « 10.4 » / « 10-4 » jugée sans risque pour un numéro de version Sonar, à noter en commentaire) ; si le projet ne porte aucune annotation de cet `id`, y pousse `Annotation { id, date: montee.date, libelle: format!("Sonar {}", montee.version), categorie: CATEGORIE_ANNOTATION_MONTEE_VERSION_SONAR.to_string(), description: None, systeme: Some(true) }` et journalise la création (`EntreeJournal`, `origine` dédiée) ; sinon ne fait rien (idempotence, dédoublonnage au stockage). Retourne la liste des identifiants créés, pour le compte-rendu de campagne.

Cet `id` non-UUID rompt l'invariant documenté du champ `Annotation.id` (« Identifiant UUID v4 de l'annotation », `src-tauri/src/modele/racine.rs`), aujourd'hui respecté par toutes les créations (`uuid::Uuid::new_v4()`). Aucun code ne valide le format de cet `id` (il est traité comme chaîne opaque côté cœur natif comme côté interface), mais la Rustdoc du champ doit alors être amendée pour couvrir les identifiants dérivés stables des annotations système. Décision à trancher explicitement à l'incrément 1 : amender la Rustdoc (retenu), ou conserver un UUID et porter le dédoublonnage au stockage sur un autre critère (couple `categorie` + `libelle`).
...
`src/app/services/sansetat/commandes/facade-commandes.service.ts` : méthode `public async interrogerMonteesVersionSonar(instance: Instance, idExterne: string, dateCiblee?: string): Promise<ResultatMonteesVersionSonar>` sur le modèle exact de `interrogerDerniereAnalyse` — signature **concrète**, aucun paramètre de type. `Instance` est un type possédé par la Façade elle-même (`services/sansetat/commandes/types-facade.ts`), pas par un Store `avecetat/etat/` : la règle de frontière unique ne s'applique donc pas ici (elle ne vise que les types possédés par les Stores, comme `DonneesRacine` pour `enregistrerBrouillon`), et les dix autres interrogations d'audit passent déjà `instance: Instance` sans généricité. Délégation à `invoke('interroger_montees_version_sonar', …)`. Types miroir `MonteeVersionSonar` et `MonteeVersionSonarProjet` dans `types-facade.ts` (`readonly`, `camelCase`).
...
Bouchons TypeScript — deux fichiers distincts :

- `bouchon-commandes.utils.ts` (bouchon d'interrogation d'audit, fallback par défaut) : nouveau `case` dans `resoudre(...)` pour `interroger_montees_version_sonar`, renvoyant pour au moins un projet du jeu de démonstration une montée fixe (`{ version: '10.4', date: <date fixe antérieure au premier audit du jeu> }`), et **la même version pour deux projets d'une même instance** (assertion de dédoublonnage E2E) ; ajout au `Set` `COMMANDES_INTERROGATION_AUDIT` pour bénéficier du délai artificiel fixe qui exerce l'indicateur de progression.
- `bouchon-administration.utils.ts` (`BouchonAdministrationUtils`, où vit `enregistrer_brouillon`) : le cas `enregistrer_brouillon` accepte et applique le nouveau paramètre `monteesVersionSonarParProjet` (upsert des annotations dans la racine bouchonnée). En-tête du fichier (« huit commandes ») et son spec ajustés en conséquence.

Aucune assertion de « décompte de commandes reconnues » n'existe dans les specs de ces bouchons ; ne pas en introduire.

### Orchestrateur de campagne — `src/app/services/avecetat/campagne/orchestrateur-campagne.service.ts`

La boucle par source Sonar est dans `auditerProjet`, méthode exécutée **en concurrence** (`mergeMap(..., concurrence)`) et qui **retourne** un objet par projet (`resultatBrouillon`, `verdict`…). Les montées suivent le même chemin : `auditerProjet` appelle `interrogerMonteesVersionSonar(instance, source.idExterne)` à côté de `interrogerDerniereAnalyse`, fusionne les montées de ses sources Sonar (dédoublonnage sur `version`) et les fait **remonter dans sa valeur de retour** (`montees?: readonly MonteeVersionSonar[]`), plutôt que par mutation d'une `Map` partagée entre exécutions concurrentes. En cas d'échec de l'appel, une anomalie est poussée sous une clé de pseudo-indicateur `sonar.montees_version` et la campagne se poursuit (jamais bloquante) ; l'appel est ignoré si la source Sonar est elle-même injoignable (anomalie déjà enregistrée pour un autre indicateur de la même source).

`lancerCampagne` agrège les retours de `auditerProjet` en un `MonteeVersionSonarProjet[]` (projets sans montée omis) et le passe à `DonneesApplicationService.enregistrerBrouillon`.
...
Les repères ne sont **pas** posés via `creerAnnotation` : ils le sont par `synchroniser_annotations_montee_version` au sein de `enregistrerBrouillon`. Conséquence assumée du point d'accrochage retenu : les annotations sont posées dès l'enregistrement du brouillon, donc **un rejet ultérieur du brouillon (`rejeterBrouillon`) ne les retire pas** — elles ne font pas partie du différentiel d'audit annulé, et une montée de version est un fait réel indépendant de la campagne. Elles seraient de toute façon reposées à la campagne suivante (idempotence).

La Fiche projet et la Comparaison d'audits, qui listent déjà `projet.annotations` en texte (catégorie affichée), montreront les nouvelles annotations `monteeVersionSonar` sans modification ; leur `systeme` étant vrai, aucun bouton de suppression n'apparaît (`@if (!annotation.systeme)` déjà en place sur la Fiche projet).
...
- `categoriesLignesPresentes: Signal<readonly CategorieLigneVerticale[]>` : `computed` dérivé de `lignesVerticales()` (ordre stable défini par un tableau statique), pour n'afficher un bouton de bascule que si la catégorie est effectivement présente.
...
Panneau de bascule à côté de la légende des séries : un `@for (categorie of categoriesLignesPresentes(); track categorie)` produisant un `<button>` avec `[attr.aria-pressed]` — **même idiome d'accessibilité que la bascule de séries existante** (`graphique-evolution.component.html`, boutons `(click)` + `aria-pressed`), et non une case à cocher, pour n'avoir qu'un seul pattern dans le composant. `(click)="basculerCategorieLigne(categorie)"`, libellé par `LIBELLE_CATEGORIE_LIGNE_VERTICALE`, puce de couleur reprise de `STYLE_LIGNE_VERTICALE`. Aucune couleur métier codée dans le SCSS.
...
Le champ `id` d'une `Annotation` porte aujourd'hui une Rustdoc « Identifiant UUID v4 de l'annotation », respectée par toutes les créations existantes (`uuid::Uuid::new_v4()`). L'identifiant dérivé stable retenu ici (`montee-version-sonar-<empreinte de la version>`) n'est pas un UUID : à l'incrément 1, amender cette Rustdoc pour couvrir les identifiants dérivés des annotations système (retenu), ou porter le dédoublonnage au stockage sur le couple `categorie` + `libelle` en conservant un UUID. Aucun code ne valide le format de cet `id` (chaîne opaque côté cœur natif comme côté interface), le choix est donc sans impact fonctionnel — seule la documentation du champ est concernée.

## 8. Impacts documentaires

| document | nature de la mise à jour |
|---|---|
| [`04_casUsage.md`](../02_documentation/04_casUsage.md) | Nouvelle ligne US-062 : persona Camille, type Consultation, priorité Could have ; critères d'acceptation = repères verticaux violets de montée de version Sonar sur le graphique de la Synthèse graphique, alimentés par les campagnes, un repère par version (dédoublonné entre projets d'une instance), panneau de masquage des repères par catégorie (état de session), repères également listés en texte sur la Fiche projet et la Comparaison d'audits via les annotations système ; RG associée RG-062. Mise à jour de la matrice de couverture. |
| [`05_reglesGestion.md`](../02_documentation/05_reglesGestion.md) | Nouvelle RG-062 : source (`project_analyses/search`, événements `SQ_UPGRADE`), matérialisation en annotation système de portée projet catégorie `monteeVersionSonar`, libellé « Sonar » suivi du numéro de version, date = analyse ayant suivi la montée, idempotence par identifiant dérivé de la version, dédoublonnage inter-projets à l'affichage par version (date la plus ancienne retenue), masquage des repères verticaux par catégorie (état de session), collecte y compris en audit historique. Extension de **RG-033** : l'annotation système `monteeVersionSonar` n'est pas supprimable, comme l'annotation `politiqueIA` de RG-015. Articulation avec RG-033, RG-015, US-016, RG-021, RG-046, RG-017. Rattachement de RG-062 aux écrans « Synthèse graphique », « Fiche projet », « Comparaison d'audits » dans la matrice écrans. Complément des matrices de traçabilité RG × US. |
| [`08_arborescenceNavigation.md`](../02_documentation/08_arborescenceNavigation.md) | Ajout de `US-062` à la ligne `Synthèse graphique` (et, pour l'affichage texte, aux lignes `Fiche projet` et `Comparaison d'audits`) de la matrice écrans / user stories. |
| [`09_maquettes.md`](../02_documentation/09_maquettes.md) | Section `### Synthèse graphique` : repère vertical violet tireté étiqueté « Sonar 10.4 » ; panneau de boutons à bascule « Afficher les repères » (un par catégorie présente, `aria-pressed`, même idiome que la bascule de séries) à côté de la légende des séries. |
...
- Test d'intégration `#[ignore]` `interroger_montees_version_contre_une_vraie_instance_sonar` (`connecteurs::tests_integration_reelle`, réutilisant `SQM_TEST_SONAR_URL`/`SQM_TEST_SONAR_TOKEN`/`SQM_TEST_SONAR_PROJET_CLE` déjà existantes, aucune nouvelle variable d'environnement) : lève la réserve documentée sur la valeur exacte du filtre `category=SQ_UPGRADE` et la forme du champ `name` — approche validée par l'utilisateur le 2026-09-08 en remplacement d'un blocage sur ce point. Le test réussit même sans aucune montée détectée (`Ok(vec![])` n'est pas une anomalie) ; la vérification de la forme réelle des événements reste **humaine** (`cargo test -- --ignored --nocapture`), à mener avant toute release contre une instance ayant réellement subi une montée de version.
- Tests unitaires Jest, nouveaux ou complétés : `facade-commandes.service.spec.ts`, `orchestrateur-campagne.service.spec.ts` (remontée des montées dans le retour de `auditerProjet`, agrégation et transmission à `enregistrerBrouillon`, anomalie `sonar.montees_version` non bloquante), `graphique-evolution.component.spec.ts`, `synthese-graphique.component.spec.ts`, `bouchon-commandes.utils.spec.ts` (`interroger_montees_version_sonar`), `bouchon-administration.utils.spec.ts` (nouveau paramètre de `enregistrer_brouillon`).
- Test de bout en bout Playwright (`e2e/parcours-complet.spec.ts`) : après la campagne du parcours, à l'étape Synthèse graphique — assertion sur la présence d'un repère `monteeVersionSonar` (fourni par le bouchon), désactivation de son bouton dans le panneau de bascule → le repère disparaît, réactivation → il réapparaît ; le dédoublonnage est couvert si deux projets bouchonnés partagent la version.
...
1. **Documents normatifs** (section 8) : livrés et validés avant tout code ; confirmation que `US-062` et `RG-062` sont libres au moment de la qualification.
...
6. `npm run build` puis `cd src-tauri && cargo build --locked`.
7. `npm start` (front seul, façade bouchonnée) : lancer une campagne sur un groupe du jeu de démonstration comportant une source Sonar, intégrer le brouillon, ouvrir **Synthèse graphique** — repère vertical violet « Sonar 10.4 », un seul repère même en affichant plusieurs projets de l'instance, désactivation puis réactivation du bouton « Montées de version Sonar » du panneau, annotation `monteeVersionSonar` visible en texte sur la Fiche projet sans bouton de suppression.
8. `npm run test:e2e` : parcours complet incluant la nouvelle étape.
