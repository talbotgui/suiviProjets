...
| [`02_documentation/15_normesSecurite.md`](../02_documentation/15_normesSecurite.md) | retirer la mention de la référence de groupe GitLab saisie sur l'écran et de sa revalidation en forme côté cœur natif (champ supprimé) ; le prérequis de portée de jeton fondé sur le roster (`GET /groups/:ref/members/all`, jeton d'administration) ne s'applique plus tel quel et doit être reformulé pour la recherche individuelle par username (`GET /users?username=<exact>`) |
| [`guide-utilisateur.md`](../guide-utilisateur.md) | paragraphes de la section « Suivre la régularité des poussées : les commits des membres » : retirer la mention de la saisie de la référence de groupe GitLab et du filtre de statut ; adapter la description de la source (membres connus `interne`/`username` actifs) et du prérequis de portée de jeton |
...
**Complété le 2026-09-16, à la réalisation de l'incrément 1** : ce tableau omettait `15_normesSecurite.md` et `guide-utilisateur.md`, tous deux touchés en prose (hors identifiants techniques littéraux) par le retrait du champ « Groupe GitLab » et de la classification par roster — omission révélée par la relecture isolée de cet incrément (deux passages laissés obsolètes dans `09_maquettes.md` également, corrigés dans le même incrément). Ces deux documents ont été mis à jour au même incrément. Le prérequis de portée de jeton pour la nouvelle recherche par username (`GET /users?username=<exact>`) n'est pas déterminé par ce plan et reste un point ouvert (cf. §15, nouveau point 5) : les deux documents se limitent, sur ce point précis, à retirer l'affirmation devenue fausse (prérequis fondé sur le roster), sans la remplacer par une nouvelle affirmation non vérifiée.
...
5. **Prérequis de portée du jeton GitLab pour la recherche par username, non déterminé.** Le prérequis antérieur documenté dans `15_normesSecurite.md` et le guide utilisateur (jeton d'administration requis pour une vue complète du roster `GET /groups/:ref/members/all`) ne s'applique plus tel quel une fois ce roster retiré (§4). Le prérequis de portée pour `GET /users?username=<exact>` (recherche globale) n'est pas déterminé par ce plan — affirmation technique précise à vérifier avant d'être présentée comme acquise, cf. [01-usage-ia-et-conventions.md#discernement](../../.claude/rules/01-usage-ia-et-conventions.md#discernement) — et reste à documenter par un humain, idéalement avant la clôture de l'incrément 2 (Connecteur Rust) qui implémente cet appel (ajouté le 2026-09-16, omission révélée par la relecture isolée de l'incrément 1).

**Vérification effectuée le 2026-09-16** (relecture demandée par l'utilisateur, sans aucun développement associé) : les Parties A à F de `plan_20_triMembresConnusEtDatesCalendaires.md` sont intégrées (US-063 à US-065, RG-061/RG-063/RG-065, commits `194996a`/`ca3f63f`/`e971b6c`). Le modèle `MembreConnu` (`racine.rs`, `types-donnees.ts`) conserve exactement les champs et valeurs supposés par ce plan — `statut` (`interne`/`client`/`partenaire`), `typeCritere` (`username`/`email`/`domaineEmail`), `partiLe` optionnel et indépendant du statut (RG-061). Le code actuel de l'écran, du Store, du Moteur de jugement, de la commande de la Façade et du Connecteur GitLab correspond, fonction par fonction et signature par signature, à l'état « avant » décrit aux §4 à §9 (`preparer_analyse_commits_membres`/`PreparationAnalyseCommitsMembres`, `lister_membres_groupe`/`lister_projets_groupe`/`ProjetGroupeGitlab`/`reference_groupe_valide`, `referenceGroupeGitlab`/`FiltreStatut`/`filtreStatut`/`libelleStatut`, `StatutDeveloppeur`/`resoudreStatut`/`reglesMembresConnus`, `BouchonCommitsMembresUtils.preparerAnalyse()`, tous présents et inchangés). Les fixtures du test de bout en bout (`e2e/donnees-test.ts`) confirment aussi la prémisse du §12 : `MEMBRE_QUALIFIE_ANTICIPATION` (`domaineEmail`/`interne`) et `MEMBRE_A_QUALIFIER_DEPUIS_ALERTE` (`username`/`partenaire`) sortiraient bien du périmètre analysé après cette évolution. Le seul commit touchant ces fichiers depuis la rédaction du plan (`e971b6c`, 2026-09-15) est une correction de flakiness d'un test Jest (heure UTC fixe des événements de poussée), sans rapport avec ce plan. **Conclusion : aucune correction de fond n'est nécessaire ; ce plan reste exécutable en l'état.**
