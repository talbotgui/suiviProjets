...
16. **Format de `premierCommitInterne.date` : date calendaire UTC `AAAA-MM-JJ`.** Le connecteur tronque `committed_date` (datetime ISO 8601 renvoyé par l'API) à sa date calendaire **en UTC**. C'est cette forme — celle de la `date` d'un audit **historique** ; un audit régulier porte, lui, un horodatage complet (`new Date().toISOString()`) — qui rend l'égalité stricte de la décision 4 fiable : en pratique, seule une campagne historique ciblant exactement cette date produit un audit correspondant (précision R18-Q-01, `plan_18_relecture.md`) (RG-058, RG-059).
...
Quand `calculerPriseEnCharge` est vrai, pour chaque projet du périmètre **dont l'audit a produit au moins un résultat** (un projet en échec total d'audit ne fait l'objet d'aucun calcul — décision utilisateur du 2026-09-07, constat R18-W-06 de [`plan_18_relecture.md`](./plan_18_relecture.md) : une campagne en échec total ne crée de toute façon aucun brouillon, et le parcours de pagination GitLab serait dépensé pour rien) :
...
- **échec absorbé par projet** : une anomalie sur le calcul de prise en charge n'échoue jamais la campagne ni l'audit du projet (même principe que RG-046 / RG-021) ; l'anomalie est consignée dans le rapport d'anomalies du brouillon, avec la **catégorie d'anomalie d'origine** du connecteur (précision R18-Q-02).
...
Retenu dans ce plan (décision 10). Cohérent avec le lien « Qualifier ce membre » déjà présent sur la ligne d'un membre inconnu (`fiche-projet.component.html`, gabarit `ligneMembre`) : sur la ligne d'un membre `interne` **dont le statut est résolu contre une règle de type `username` ou `email`** (pas `domaineEmail`) **et ne portant pas déjà de `partiLe`** (rien à faire sinon — précision R18-Q-08, `plan_18_relecture.md`), un lien « Marquer comme parti » ouvrant l'écran d'administration pré-filtré sur cette règle, `partiLe` pré-rempli à la date du jour, laissé à ajuster puis à enregistrer (jamais d'écrasement d'une date déjà enregistrée). Réutilise le mécanisme de `queryParamsQualification` déjà en place. Aucune mise en évidence d'accès résiduel n'est ajoutée ici (plan_19).
...
Incréments 1 à 7 : **développés et relus** (relecture isolée du 2026-09-07, [`plan_18_relecture.md`](./plan_18_relecture.md) ; corrections à l'[Étape 38 du rapport](../04_rapports/rapportDeDeveloppement.md)). Palier de migration : **fixé à `10` → `11`** (`plan_17` chapitre 4 non encore intégré). Le paramètre `parti_le` des commandes `qualifier_membre` / `qualifier_membres` a été retenu (§8.2) ; le type d'erreur de `calculerPriseEnChargeProjet` est `ErreurConnecteur` (aligné sur `commandes/audit.rs`, écart assumé au pseudo-code §4.3, cf. R18-W-07).

Incrément 8 : la **documentation utilisateur** (section « Date de prise en charge » de `guide-utilisateur.md`) a été produite avec l'étape documentaire (commit `plan 18 - docs`) ; le **parcours E2E** (`e2e/parcours-complet.spec.ts`) et la **revue croisée finale de la matrice de traçabilité** ont été réalisés le 2026-09-07 (cf. §14 point 6). Parcours E2E : case « Options » (décochée par défaut → cochée) à la campagne, métadonnée « Âge chez nous » et bouton « Recalculer » de la Fiche projet (notification « inchangée » sans ressaisie de mot de passe), raccourci « Depuis la prise en charge » de la Comparaison (désactivé + invite, aucune campagne historique dans le parcours). Revue croisée : US-058/059/061 ↔ RG-058/059/061 ↔ matrice écrans×US (`08`) ↔ conception (`13`) ↔ modèle (`12`) ↔ tests (`16`) ↔ maquettes (`09`) ↔ Specification §5.17 — cohérence vérifiée. **Reste à faire (manuel, hors pipeline)** : régénération des deux captures `docs/assets/captures/constitution-campagne.png` (carte « Options » visible) et `docs/assets/captures/fiche-projet.png` (bouton « Recalculer » de « Âge chez nous » visible) via le script Playwright local non versionné (procédure `17_posteDeveloppeur.md`), viewport `1720 × 900`, jeu de démonstration du bouchon TypeScript.
...
3. **Repli sur dépôt volumineux** (§4.1, cas `x-total-pages` absent) : le parcours avant borné n'inspecte que les commits récents. Un test d'intégration `#[ignore]` dédié (`rechercher_premier_commit_interne_contre_une_vraie_instance_gitlab`, cf. R18-E-02) permet de l'éprouver sur un vrai gros dépôt via `SQM_TEST_GITLAB_PROJET_GROS_ID` ; reste à confirmer sur données réelles que le comportement `indetermine_trop_de_commits` est acceptable, ou décider d'une stratégie de parcours arrière par `page = total estimé` (l'API accepte un numéro de page arbitraire et renvoie une page vide au-delà).
...
6. **Relecture isolée** : conduite le 2026-09-07, consignée dans [`plan_18_relecture.md`](./plan_18_relecture.md) ; corrections reportées à l'[Étape 38 du rapport de développement](../04_rapports/rapportDeDeveloppement.md). Décisions utilisateur du 2026-09-07 : en échec total d'audit, aucun calcul de prise en charge n'est lancé (R18-W-06) ; le masquage inter-canaux non reproduit par la table de correspondance est un écart acceptable (R18-W-03) ; la régénération des captures du guide utilisateur est planifiée à l'incrément 8 (R18-Q-10).
