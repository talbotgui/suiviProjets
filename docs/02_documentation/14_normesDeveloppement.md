...
Cette stratégie de branches et la convention de commits ci-dessous s'appliquent aux commits de code applicatif, à partir de la mise en place du poste de développeur (étape 12) ; les commits de la phase de cadrage documentaire déjà réalisés suivent le format libre en usage jusqu'ici et ne sont pas repris rétroactivement.
...
- L'audit de vulnérabilités est outillé (`cargo audit` côté Rust, `npm audit` côté Angular/npm) ; son intégration en continu est détaillée à l'étape 12, les normes de sécurité associées en [Analyse des dépendances vulnérables](./15_normesSecurite.md#analyse-des-dépendances-vulnérables) (étape 10).
...
- Les extensions d'éditeur recommandées (analyse statique, formatage, support Rust/Angular) sont déclarées dans `.vscode/extensions.json`, déjà présent dans le dépôt et complété au fil des choix technologiques ; leur activation effective au poste développeur reste détaillée à l'étape 12.
