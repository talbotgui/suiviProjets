...
| Génération et publication de la couverture de code | `cargo-llvm-cov --html` (seuil global 80 %) et `jest --coverage` (seuils par périmètre déjà configurés, reporter `html`) assemblés en un site statique unique, déployé sur GitHub Pages à chaque exécution du workflow (cf. [Publication](#publication) ci-après) |
