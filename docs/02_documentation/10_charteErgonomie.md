...
| Modale de confirmation | Rappelle explicitement l'action et sa conséquence, bouton de validation jamais pré-sélectionné par défaut pour une action destructive ; fermeture par Échap (équivaut au bouton Annuler, sans effet pendant le traitement d'une confirmation en cours) |
...
| Bouton de copie rapide | Action de copie d'un texte dans le presse-papiers (`navigator.clipboard`), avec confirmation visuelle explicite du succès de la copie ; usage initial : un identifiant technique affiché (identifiant de dépendance, motif, etc.) ; libellé visible et infobulle survolable facultative paramétrables par l'appelant, ce qui permet aussi de s'en servir comme bouton d'aide « copier un exemple de prompt » (modale de saisie en masse de règles de dépendances, [US-043](./04_casUsage.md#cas-dusage--user-stories)) ; composant transverse, destiné à être réutilisé tel quel ailleurs dans l'application ([US-042](./04_casUsage.md#cas-dusage--user-stories), besoin identifié en Phase 15 (C15-06), absent de `Specification.md`) |
| Modale de saisie en masse | Superposition avec zone de texte collé (une ligne par entrée), boutons Annuler/Valider, affichage des erreurs ligne par ligne sans bloquer la validation des lignes correctes ; fermeture par Échap (équivaut au bouton Annuler, sans effet pendant le traitement d'une soumission en cours) ; squelette transverse partagé entre plusieurs entités (règles de dépendances, membres connus), chaque appelant fournissant sa propre logique de parsing et de validation, ainsi que le texte d'aide projeté (grammaire attendue et, pour les règles de dépendances, bouton de copie d'un exemple de prompt IA) ([US-043](./04_casUsage.md#cas-dusage--user-stories), [US-044](./04_casUsage.md#cas-dusage--user-stories), besoin identifié en Phase 15 (C15-07, C15-08), absent de `Specification.md`) |
...
- toute superposition modale disposant d'une action d'annulation légitime se ferme par la touche Échap, en cohérence avec le comportement de la recherche globale (la superposition plein écran de session verrouillée en est exclue : elle n'offre aucune annulation).
