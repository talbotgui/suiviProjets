...
Le dossier source mentionnait, à titre indicatif, un cœur Rust, une interface Angular (Signals, RxJS) et un packaging Tauri (cf. [Specification.md, section 1.1](../01_besoin/Specification.md#11-architecture-retenue)), ainsi que la chaîne cryptographique zstd/Argon2id/AES-256-GCM (cf. [Specification.md, section 6.3](../01_besoin/Specification.md#63-enveloppe-chiffrée)) : ces choix sont **confirmés** ci-dessous, pour les raisons détaillées dans la colonne « justification ». Le choix d'ECharts et celui du rendu DOM vers image pour les exports hors graphiques sont, à l'inverse, des choix **nouveaux**, non suggérés dans le dossier source.
...
| Interface utilisateur en [Angular](https://angular.dev/) (TypeScript), [Signals](https://angular.dev/guide/signals) pour l'état réactif local, [RxJS](https://rxjs.dev/) pour l'orchestration asynchrone | Typage fort adapté à un modèle de données riche et versionné ([26 fonctionnalités](../01_besoin/Specification.md#3-résumé-des-fonctionnalités) interdépendantes) ; Signals adaptés à la réactivité fine des tableaux denses et du tableau de bord temps réel ; RxJS adapté à l'orchestration de campagnes d'audit concurrentes |
