...
|---|---|---|
| Cœur natif — Connecteur GitLab | Une opération par type de résultat (`interrogerDependances`, `interrogerBranches`, `interrogerVitalite`, `interrogerContributeurs`, `interrogerTailleDepot`, `interrogerMergeRequests`, `interrogerMarqueursIA`, `interrogerMembres`) ainsi que `testerConnectivite(instance, credential)` | Client HTTP ([reqwest](https://docs.rs/reqwest/)), configuration proxy du poste, credential en mémoire fourni par l'appelant |
| Cœur natif — Connecteur Sonar | `interrogerViolations`, `interrogerDette`, `interrogerCouverture`, `interrogerNotes`, `interrogerNcloc`, `testerConnectivite` | Client HTTP (reqwest), configuration proxy, credential en mémoire fourni par l'appelant |
| Cœur natif — Connecteur croisé | `calculerFraicheurSonar`, `calculerActiviteSansQualite`, `calculerIaNouveauCode` | Résultats déjà obtenus des connecteurs GitLab et Sonar pour le même audit (aucun appel réseau propre) |
| Cœur natif — Moteur de persistance | `creerFichier(chemin, motDePasse)`, `chargerFichier(chemin, motDePasse)`, `sauvegarderFichier(chemin, données, motDePasse)`, `migrerSchema(données)` | RustCrypto (Argon2id, AES-256-GCM), zstd, système de fichiers du poste |
| Cœur natif — Orchestrateur de campagne | `constituerCampagne(périmètre)`, `lancerCampagne(campagne, credentials)` (retourne un flux d'événements de progression), `annulerCampagne(campagneId)` | Connecteur GitLab, Connecteur Sonar, Connecteur croisé, Moteur de persistance (écriture du brouillon) |
| Cœur natif — Façade de commandes | Ensemble des commandes mutantes (`creerFichier`, `chargerFichier`, `sauvegarderFichier`, `lancerCampagne`, `annulerCampagne`, `integrerBrouillon`, `rejeterBrouillon`, `qualifierMembre`, `definirPolitiqueIA`, `verrouillerSession`, `creerAnnotation`, `qualifierAlerte`, `definirSeuil`, `definirReferentiel`, …) et des événements poussés (`progressionCampagne`, `verrouillageDeclenche`) | Tous les modules du cœur natif ci-dessus |
...
| UI — Écrans et navigation | Composants d'écran de l'arborescence (étape 5) et routeur applicatif associé | Store d'état applicatif, Moteur de jugement, Façade de commandes |
...
1. L'écran de constitution de campagne invoque `constituerCampagne(périmètre)` ; l'Orchestrateur de campagne calcule le coût prévisionnel et vérifie, via les credentials transmis par le Store d'état applicatif, que rien ne manque.
2. L'UI invoque `lancerCampagne(campagne, credentials)`.
3. L'Orchestrateur de campagne planifie les appels avec une concurrence limitée ([RG-017](./05_reglesGestion.md#audits-et-campagnes)) ; pour chaque projet du périmètre, il invoque le Connecteur GitLab et le Connecteur Sonar, puis le Connecteur croisé une fois les deux résultats disponibles.
4. À chaque projet terminé, l'Orchestrateur de campagne pousse un événement `progressionCampagne` à travers la Façade de commandes ; le Tableau de bord d'exécution se met à jour de façon réactive (Store d'état applicatif).
5. À l'issue de la campagne, l'Orchestrateur de campagne écrit le Brouillon (`resultatsParProjet`) via le Moteur de persistance et signale les valeurs aberrantes par comparaison avec le dernier audit intégré de chaque projet ([RG-020](./05_reglesGestion.md#audits-et-campagnes)).
...
| Cœur natif — Connecteur croisé | [RG-013](./05_reglesGestion.md#constat-jugement-et-politique-ia), [RG-016](./05_reglesGestion.md#constat-jugement-et-politique-ia) |
