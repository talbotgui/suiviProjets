...
| Temps disponible pour un développement solo insuffisant face à l'ampleur du périmètre ([F01 à F26](../01_besoin/Specification.md#3-résumé-des-fonctionnalités)) | Retard indéfini ou abandon du projet | Moyenne | Étapes de cadrage validées indépendamment les unes des autres, priorisation MoSCoW des cas d'usage à l'étape 3 |
| Évolution des contrats d'API GitLab ou Sonar | Rupture ponctuelle d'un ou plusieurs connecteurs | Faible à moyenne | Test de connectivité dédié ([F24](../01_besoin/Specification.md#524-f24--test-de-connectivité-global)), anomalies typées et actions suggérées ([F08](../01_besoin/Specification.md#58-f08--rapport-danomalies-techniques-daudit)) |
