...
| `SQM_TEST_GITLAB_DOMAINE_INTERNE`, `SQM_TEST_GITLAB_PROJET_GROS_ID` | Optionnelles, propres au test d'intégration hors CI `rechercher_premier_commit_interne` (plan_18, US-058/RG-058) : domaine d'un auteur de commit interne du projet testé, et identifiant d'un dépôt de plus de ~10 000 commits pour éprouver le repli en parcours avant (en-tête `x-total-pages` omis par l'API) | Définies localement par le développeur, jamais committées |
