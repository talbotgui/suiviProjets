# Exigences non fonctionnelles

## Sommaire

1. [Convention d'identifiant](#convention-didentifiant)
2. [Performance](#performance)
3. [Scalabilité et montée en charge](#scalabilité-et-montée-en-charge)
4. [Disponibilité et tolérance aux pannes](#disponibilité-et-tolérance-aux-pannes)
5. [Sécurité](#sécurité)
6. [Accessibilité](#accessibilité)
7. [Portabilité et environnements cibles](#portabilité-et-environnements-cibles)
8. [Internationalisation et localisation](#internationalisation-et-localisation)
9. [Contraintes réglementaires et légales](#contraintes-réglementaires-et-légales)
10. [Matrice de traçabilité](#matrice-de-traçabilité)

## Convention d'identifiant

*Ce chapitre définit la convention d'identifiant stable attribuée à chaque exigence non fonctionnelle (par exemple RNF-xxx), réutilisée sans renommage dans les étapes suivantes.*

## Performance

*Ce chapitre décrit les exigences de performance attendues : temps de réponse cibles et volumétrie attendue.*

| identifiant | exigence | valeur cible |
|---|---|---|
| | | |

## Scalabilité et montée en charge

*Ce chapitre décrit les exigences de scalabilité et de montée en charge attendues, distinctes des exigences de performance.*

| identifiant | exigence |
|---|---|
| | |

## Disponibilité et tolérance aux pannes

*Ce chapitre décrit les exigences de disponibilité de l'application et sa tolérance aux pannes.*

| identifiant | exigence |
|---|---|
| | |

## Sécurité

*Ce chapitre décrit les exigences de sécurité attendues : authentification, autorisation, protection des données.*

| identifiant | exigence |
|---|---|
| | |

## Accessibilité

*Ce chapitre décrit les exigences d'accessibilité attendues, y compris les paliers de largeur de fenêtre auxquels l'application reste pleinement utilisable sans perte de contenu ni de fonctionnalité (critère de reflow WCAG 2.1, 1.4.10) : ces paliers, une fois fixés ici avec une valeur chiffrée, sont repris tels quels par la [charte visuelle](./10_charteErgonomie.md#comportement-responsive) de l'étape 5 plutôt que redéfinis à cette étape ultérieure.*

| identifiant | exigence | valeur cible |
|---|---|---|
| | | |

## Portabilité et environnements cibles

*Ce chapitre décrit les environnements cibles sur lesquels l'application doit fonctionner et les exigences de portabilité associées.*

| identifiant | exigence |
|---|---|
| | |

## Internationalisation et localisation

*Ce chapitre décrit les exigences d'internationalisation et de localisation applicables, le cas échéant : langues supportées, formats régionaux (dates, devises, unités).*

| identifiant | exigence |
|---|---|
| | |

## Contraintes réglementaires et légales

*Ce chapitre décrit les contraintes réglementaires ou légales applicables, notamment en matière de protection des données personnelles.*

## Matrice de traçabilité

*Ce chapitre relie chaque exigence non fonctionnelle aux objectifs du projet ou aux cas d'usage concernés.*

| exigence non fonctionnelle | objectif / cas d'usage concerné |
|---|---|
| | |
