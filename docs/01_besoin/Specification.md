...
- **Projets** : nom, description, politique IA (cf. F18), qualification « en stase » (case à cocher, `false` par défaut — projet volontairement mis en pause, sans effet sur l'audit, les campagnes, les alertes ni le calcul des indicateurs, restitué en fond gris clair sur les écrans Obsolescence et Synthèse des audits et par une pastille sur la Fiche projet, ajouté le 2026-09-15, cf. [plan_20](../03_plan/plan_20_triMembresConnusEtDatesCalendaires.md)), sources rattachées, duplication d'un projet existant (mêmes instances, même paramétrage, jamais la qualification « en stase ») pour accélérer la saisie en masse.
...
L'application peut calculer, **à la demande uniquement** (case à cocher du formulaire de constitution de campagne, décochée par défaut, ou bouton « recalculer » de la Fiche projet), la **date de prise en charge** d'un projet, c'est-à-dire la date du premier commit interne (cf. F17). Les suppressions sont confirmées et rappellent la perte de l'historique d'audits associé.
...
| `gitlab.membres` | Membres (username, nom, niveau d'accès effectif ; `direct` = membre nominatif du dépôt ; `groupesInvites` = chemins complets des groupes invités au projet dont le membre relève) et auteurs de commits | Statuts interne / client / partenaire / inconnu via les membres connus (cf. F17) ; ventilation directs / groupes invités / hérités de l'arborescence sur la Fiche projet (cf. F12) |
...
Écran de suivi temps réel de chaque campagne : progression globale, estimation de durée restante (moyenne glissante), et liste des projets avec état — *en attente*, *en cours* (connecteur actif affiché), *terminé* (durée, nombre de résultats), *échoué* (motif court : lorsque l'échec provient d'un appel de connecteur, le code technique RG-021 est restitué via le libellé lisible partagé avec F08, le tag de l'indicateur concerné restant affiché en préfixe), *ignoré*. Les échecs n'interrompent pas la campagne. L'**annulation est propre** : elle vide la file sans interrompre les requêtes en vol ; les résultats acquis sont conservés, les projets non traités deviennent *ignorés* et rejoignent le périmètre de reprise. La concurrence est limitée (paramétrable, défaut 4 projets simultanés) pour ménager les instances et le proxy. Pendant une campagne, le minuteur d'inactivité du verrouillage est suspendu.
...
Visualise tous les indicateurs de tous les audits d'un projet, avec une vue graphique de l'évolution de chacun. En tête : identité du projet, politique IA, date du premier commit interne (« âge du projet chez nous »), dernier audit et son verdict. Sections : indicateurs par thème, membres et statuts, marqueurs IA détectés, section « IA & qualité du nouveau code » (courbes juxtaposées, ligne verticale à la date d'activation IA), annotations, entrées de journal concernant le projet. Export PNG de la fiche.

**Membres et statuts** : les membres du dépôt sont ventilés en trois sections repliables, fermées par défaut, chaque barre de titre portant le décompte par statut (statuts sans membre omis) : (1) membres nominatifs directs du dépôt ; (2) membres des groupes invités au projet, regroupés par groupe (chemin complet mentionné une seule fois), les groupes triés du plus précis vers la racine ; (3) membres hérités de l'arborescence du projet (le groupe ancêtre précis n'est pas recherché). Un même membre peut figurer dans plusieurs sections (ex. direct **et** membre d'un groupe invité). Chaque ligne de membre est identique d'une section à l'autre (nom, niveau d'accès effectif, badge de statut, gravité et raccourci « qualifier ce membre » en deux clics vers les membres connus du groupe pour un membre inconnu). Les traitements « membre inconnu » (badge d'en-tête, seuil du lien « créer des règles en masse ») portent sur l'ensemble des trois sections. L'export PNG déplie toujours les trois sections. Si le détail du projet ou la liste des membres d'un groupe invité est inaccessible, les membres concernés retombent en section (3), sans erreur visible.
...
**Premier commit interne (date de prise en charge)** : le calcul a lieu **uniquement sur action explicite** — case à cocher (décochée par défaut) d'une carte « Options » du formulaire de constitution de campagne, ou bouton « recalculer » de la Fiche projet ; il n'existe aucun déclencheur automatique (la mention antérieure d'un calcul « à la première liaison d'une source GitLab » est abandonnée). L'application recherche le premier commit, sur l'une des sources GitLab du projet, dont le courriel d'auteur correspond à une règle de membre connu de statut `interne` du groupe — précédence de correspondance reprise de la liste des membres connus mais restreinte aux canaux exposés par un commit (courriel exact, alias courriel, domaine de courriel ; le username n'étant pas exposé par l'API des commits, une règle `interne` de type `username` sans alias courriel reste sans effet ici). La recherche pagine `repository/commits` depuis la dernière page (l'API ne trie pas du plus ancien au plus récent) ; elle est bornée à `parametres.audit.borneRecherchePremierCommitPages` pages par source (défaut 50). Lorsque plusieurs sources GitLab existent, la **date la plus ancienne** obtenue source par source est retenue. La date stockée est une **date calendaire UTC** (`AAAA-MM-JJ`), au même format que la `date` d'un audit **historique** (un audit régulier porte un horodatage complet) : en pratique, seule une campagne historique ciblant exactement cette date produit un audit dont la `date` lui est strictement égale (cf. écran de comparaison entre deux audits).

Le résultat est stocké comme attribut **stable, recalculable à la demande** du projet (`date`, `SHA`, courriel d'auteur, date de calcul, empreinte du sous-ensemble `interne` des membres connus au moment du calcul, `statut`). Il n'est **jamais recalculé automatiquement** ; un recalcul (campagne ou bouton) qui reproduit la valeur stockée (même date et même statut) **n'entraîne aucune écriture** (ni ressaisie du mot de passe, ni sauvegarde, ni entrée de journal). Statuts possibles : `determine`, `aucune_regle_interne` (aucune règle `interne` exploitable — soit aucune règle `interne` définie, soit uniquement des règles de type `username` sans alias courriel, sans effet sur un commit ; aucun appel réseau ; la fiche projet distingue ces deux cas à l'affichage et invite alors à compléter la qualification par une adresse, un alias ou un domaine), `aucun_membre_interne` (parcours complet sans correspondance), `indetermine_trop_de_commits` (fenêtre bornée épuisée, ou dépôt trop volumineux pour être remonté), `non_applicable` (aucune source GitLab exploitable), `depot_vide` (toutes les sources GitLab ont un dépôt vide). Bouton « recalculer » dans la Fiche projet (recalcul systématique), suggéré discrètement quand l'empreinte du référentiel `interne` a changé depuis le dernier calcul.

Un membre interne **ayant quitté l'organisation** (date de départ `partiLe` renseignée sur sa règle) reste pris en compte pour cette datation : ses anciens commits sont des commits internes légitimes, et `partiLe` n'entre pas dans l'empreinte du référentiel. Sur l'écran de comparaison entre deux audits, une comparaison « depuis la prise en charge » n'est possible que si un audit (régulier ou historique) du projet porte une `date` exactement égale à la date de prise en charge ; à défaut, l'utilisateur est invité à lancer une campagne historique ciblant cette date, l'application ne créant jamais cet audit d'elle-même.
...
À côté de ces deux purges automatiques, une **suppression ciblée** (US-063) présente, dans le même écran, la liste transverse des audits de **tous les projets de tous les groupes** — groupe, projet, type (régulier / historique), date ciblée, date de réalisation / création, campagne d'origine, nombre d'indicateurs —, filtrable par plage de date de réalisation / création (filtre primaire, notamment pour retirer les audits produits par des versions antérieures de l'application), plage de date ciblée, groupe, projet et type. L'utilisateur coche une sélection (ou « tout le résultat filtré »), obtient une prévisualisation du volume et de la liste des projets qui se retrouveraient sans aucun audit, puis confirme et ressaisit le mot de passe du fichier. Contrairement à la purge par densité, cette suppression n'impose aucune conservation du premier ou du dernier audit d'un projet (ajouté le 2026-09-15, cf. [plan_20](../03_plan/plan_20_triMembresConnusEtDatesCalendaires.md)).
...
│       ├── premierCommitInterne   attribut stable recalculable à la demande (statut + données)
