...
Trois agents distincts, à trois moments différents de cette phase, ont exécuté par erreur des commandes de consultation du gestionnaire de versions malgré l'interdiction explicite et répétée dans chaque consigne. Aucune n'était destructive. Un garde-fou technique de configuration bloquant ce type de commande pour l'IA a bien été mis en place depuis (comme recommandé après les deux premières occurrences), mais l'incrément 7 a révélé que son motif de blocage ne filtre que les commandes dont le premier mot est le gestionnaire de versions lui-même : une commande composée dans laquelle celui-ci apparaît de façon non initiale (par exemple une substitution de processus imbriquée dans une commande de comparaison de fichiers) n'est pas interceptée et s'exécute malgré tout. Le garde-fou existant reste donc nécessaire mais insuffisant à lui seul ; son motif de blocage doit être renforcé pour couvrir ce cas, faute de quoi la vigilance de chaque agent reste, comme avant sa mise en place, le seul rempart réel sur ce point précis.
