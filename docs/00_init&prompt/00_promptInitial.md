...
*Il doit être utilisé avec les modèles de documents, situés dans un sous-répertoire `_modèles` de chaque dossier thématique de ./docs (ex : ./docs/02_documentation/_modèles).*
...
- lorsqu'une commande de la Façade échange un type de données par ailleurs déjà possédé par un module applicatif porteur d'état (ex. la racine complète d'un fichier de données, déjà typée au sein d'un Store d'état applicatif), précise explicitement que le client de la Façade qui l'invoque en couche interface reste générique sur ce type plutôt que de l'importer, afin d'éviter qu'une implémentation ultérieure ne contourne la frontière unique vers les commandes natives au motif, en apparence seulement fondé, d'une dépendance circulaire entre les deux couches (enseignement de la Phase 4 du plan de développement d'un premier projet cadré par ce prompt : ce cas de figure, non anticipé par ce document, a conduit un Store d'état applicatif à invoquer directement les commandes natives concernées plutôt que de passer par le client typé de la Façade déjà en place, corrigé seulement en relecture) ;
...
- installation du poste de développeur et son usage (outils, configuration, exécution et tests en local) ; nomme explicitement l'outil de pilotage de navigateur headless retenu pour la vérification interactive d'un écran pendant le développement (ex : Playwright), installé comme dépendance versionnée du projet dès l'introduction du premier écran plutôt que laissé à une installation ponctuelle non versionnée le jour où le besoin apparaît ; ce point est distinct de la suite de tests de bout en bout de l'étape 11, qui s'exécute contre l'application packagée et non contre le seul serveur de développement de l'interface (enseignement de la Phase 3 du plan de développement d'un premier projet cadré par ce prompt : aucun outil de ce type n'était prévu par la documentation ni installé sur le poste, obligeant à en installer un hors du dépôt et hors du suivi de versions pour vérifier visuellement le premier écran construit) ;
...
Avant rédaction, dresse la liste exhaustive des actions tracées comme différées dans les plans de mise en place (`./docs/03_plan/plan_XX_*.md`, règle générale n°8) ainsi que des chapitres de `./docs/02_documentation` décrivant un livrable ou une stratégie de vérification distincte non encore rattaché à un cas d'usage ou une règle de gestion (ex : tests de charge et de performance de l'étape 11), afin de garantir leur reprise explicite dans une phase du plan ou leur exclusion justifiée.
Pour tout écran de l'arborescence de navigation (étape 5) dont les onglets ou sous-parties sont couverts par des cas d'usage assignés à des phases différentes du plan, précise explicitement, onglet par onglet, la phase qui le construit, et rappelle dans chaque phase ultérieure concernée qu'elle complète un écran déjà partiellement construit plutôt que d'en créer un nouveau ; réciproquement, vérifie qu'aucun écran cité dans le plan n'omet un onglet pourtant rattaché à un cas d'usage déjà couvert par une phase antérieure ou ultérieure (enseignement de la Phase 3 du plan de développement d'un premier projet cadré par ce prompt : l'écran unique « Administration » de l'arborescence de navigation regroupe des onglets couverts par les phases 3 et 4 du plan sans que cette répartition fine soit explicitée ailleurs que dans le tableau des phases par cas d'usage, obligeant le développeur à la déduire lui-même, au risque de construire par erreur un onglet relevant d'une phase non encore atteinte).
