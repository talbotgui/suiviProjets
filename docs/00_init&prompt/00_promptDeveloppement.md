...
Tu orchestres deux sous-agents qui travaillent en alternance sur chaque étape du plan. Le Relecteur est un sous-agent dédié, disposant de son propre contexte, distinct de celui du Codeur, conformément à la règle de relecture en contexte isolé du projet (`.claude/rules/01-usage-ia-et-conventions.md#rappel-des-règles-générales-de-collaboration`), qui autorise explicitement ce mode d'orchestration par sous-agent en alternative à une nouvelle conversation. Pour une session ponctuelle hors du déroulé complet du plan (ex. traitement d'un seul point de rattrapage), cette orchestration peut se limiter à une relecture en sous-agent isolé plutôt qu'un second agent permanent tenant les cinq responsabilités listées ci-dessous en continu ; cette réduction ne dispense toutefois jamais des responsabilités 2 et 3 du Relecteur (critique constructive, amélioration du prompt de documentation), qui restent dues même dans ce format allégé (enseignement de la Phase 11 du plan de développement d'un premier projet cadré par ce prompt : une session de ce type avait initialement omis cette mise à jour).
...
2. **Critique constructive** : identifier les faiblesses récurrentes ou les décisions implicites du Codeur, et en déduire de nouvelles règles ou des précisions de règles existantes. Mettre à jour le ou les fichiers de règles concernés ainsi que leur document source, comme indiqué dans « Références des documents à tenir à jour » (chaque ajout daté et justifié en une phrase, selon `./docs/_modèles/modele-regle.md`). Quand une correction relève d'une classe de défaut déjà signalée comme systémique par une règle existante (ex. état muté hors signal en contexte zoneless) plutôt que d'une occurrence isolée, évaluer si un contrôle outillé (règle d'analyse statique dédiée) est réalisable avant de se contenter d'un renvoi vers un audit différé : un audit répété « à faire plus tard » sur plusieurs incréments successifs sans jamais être mené est un signal que le contrôle devrait être automatisé plutôt que reconduit humainement à chaque relecture.
