...
- `SQM_TEST_GITLAB_URL`/`SQM_TEST_GITLAB_TOKEN`/`SQM_TEST_GITLAB_PROJET_ID`, `SQM_TEST_SONAR_URL`/`SQM_TEST_SONAR_TOKEN`/`SQM_TEST_SONAR_PROJET_CLE` : credentials et projet réel des tests d'intégration hors CI. `SQM_TEST_GITLAB_DOMAINE_INTERNE`/`SQM_TEST_GITLAB_PROJET_GROS_ID` : optionnelles, propres au test `rechercher_premier_commit_interne` (plan_18).
