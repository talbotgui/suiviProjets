...
- Le garde-fou technique de `.claude/settings.json` bloquant les commandes `git` ne couvre que les commandes dont le premier mot est `git` : une commande composée où `git` apparaît de façon non initiale (ex. substitution de processus `<(git ...)`, sous-shell) n'est pas interceptée par ce seul motif de refus ; toute commande shell soumise par l'IA doit donc être relue avant exécution pour l'absence de tout appel `git` imbriqué, ce filtrage manuel restant nécessaire tant que le motif de refus n'a pas été renforcé (ajouté le 2026-07-26 : contournement constaté en relecture de la Phase 6 incrément 7, une commande `diff <(git show ...)` exécutée par erreur par le Relecteur malgré l'interdiction explicite et malgré le garde-fou déjà en place, cf. [source](../../docs/02_documentation/01_modalitesUsageEtConventions.md#actions-à-exécution-strictement-humaine)).
